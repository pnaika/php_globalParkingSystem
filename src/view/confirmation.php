...
<?php if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['payId'])): ?>
    <!doctype html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
    ...
    <div id="wrapper">
        <div id="header-container">
            <ul class="secondary-nav">
                <li><?php print '<a href="getAllPayment.php?empId='.$employeeId.'">BACK</a>'?></li>
                <li><?php print '<a href="employeeHome.php?empId='.$employeeId.'">HOME</a>'?></li>
                <li><a href="index.php?logout=yes">LOGOUT</a></li>
            </ul>
        </div>
        <div class="alert alert-warning" role="alert">ARE YOU SURE YOU WANNA DELETE THIS PAYMENT DETAIL ??</div>
        <form action="deletePayment.php?empId=<?php print $employeeId ;?>&payId=<?php print $paymentId?>" method="POST">
            <input type="hidden" name="payId" value="<?php print $employeeId;?>">
            <input type="hidden" name="empId" value="<?php print $paymentId;?>">
            <button type="submit" class="btn btn-primary">YES</button>
            <a href="getAllPayment.php?empId=<?php print $employeeId ;?>" class="btn btn-primary">NO</a>
        </form>
        <footer>
